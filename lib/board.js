...
function load(containerElIdVal, terraControlsElIdVal, config){
	...
	if(!cfg.position || cfg.position === 'start')
      ...
      if(!cfg.boardPosition || cfg.boardPosition === 'start')
        cfg.boardPosition = START_BOARD_FEN;
	...
	if(TERRAFORMING){
		removeSquareHighlights();
            ...
            if(CURRENT_TERRAFORM_TYPE === CURRENT_BOARD_POSITION[square] ||
              TERRAFORM_OPTIONS.indexOf(square) === -1){
		...
		let lostSoldiers = cfg.terraform(CURRENT_TERRAFORM_TYPE, square);
...
function findTerraformOpts(e){
  ...
  let options = cfg.onTerraform(terraformType);
