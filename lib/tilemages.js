...
class TileMages {
	...
	generateTerraformOptions(type){
		...
		for(let i=0; i<this.board.length; i++){
			...
			let terraState = this.terraState;
			let piece = board[i];
			...
			//   me.turnActions === 0){
				...
				let allAround = oneAway.concat(twoAway);
				let available = [];

				allAround.forEach(function(space){

					let potPiece = board[SQUARES[space]];
					let terraformAny = false;
