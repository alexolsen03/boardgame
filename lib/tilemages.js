		...
		for(let i=0; i<this.board.length; i++){
			...
			if(piece &&
			   ...
			   this.turnActions === 0){
				...
				let available = calculateMoveableSquares(space, this.board, this.terraState, [], 0, 2,[], goDirection);
	...
	conductBattles(){
		...
		for(let i=0; i<this.board.length; i++){
			...
			if(piece){		// if there is a piece
				...
				let surroundingTiles = calculateMoveableSquares(indexToNotation(i), this.board, this.terraState, [], 0, 1,[], getAdjacentTiles);
