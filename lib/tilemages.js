...
class TileMages {
	...
	move(move){
		...
		let validMoves = calculateMoveableSquares(move.from, this.board,this.terraState,[],0,pieceMaxMoves,[]);
	...
	generateMoveOptions(from){
		...
		if(piece)
			return calculateMoveableSquares(from, this.board,this.terraState,[],0,pieceMaxMoves,[]);
	}
...
function findRangeTraveled(from, to, board, terraState){
	let rangeOne = calculateMoveableSquares(from, board, terraState, [], 0, 1, []);
	console.log(to + ' -> ' + rangeOne);
	if(rangeOne.indexOf(to) !== -1)
		return 1;

	let rangeTwo = calculateMoveableSquares(from, board, terraState, [], 0, 2, []);
