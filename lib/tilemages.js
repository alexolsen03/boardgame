...
//		console.log(calculateMoveableSquares('a10',this.board,[],0,2,[]).sort());
	}
...
/*		let move_obj = null;
		...
		let x = calculateMoveableSquares(move.from,this.board,[],0,2,[]);
		console.log('->');
		console.log(x);
		return x;
	}
...
/*************************
	PATHFINDING
*************************/
function calculateMoveableSquares(square,board,pathsFound,stepsTaken, MAX_STEPS, moveableSquares){
...
//  console.log('checking north');
  if(isNorthValid())
 	goDirection(board, north, localPathsFound, stepsTaken, MAX_STEPS, moveableSquares);	// NORTH

//  console.log('checking northeast');
  if(isNorthValid() && isEastValid())
  	goDirection(board, northEast, localPathsFound, stepsTaken, MAX_STEPS, moveableSquares);	// NORTH EAST

//  console.log('checking east');
  if(isEastValid())
  	goDirection(board, east, localPathsFound, stepsTaken, MAX_STEPS, moveableSquares);	// EAST

//  console.log('checking southeast');
  if(isSouthValid() && isEastValid())
  	goDirection(board, southEast, localPathsFound, stepsTaken, MAX_STEPS, moveableSquares);	// SOUTH EAST

//  console.log('checking south');
  if(isSouthValid())
  	goDirection(board, south, localPathsFound, stepsTaken, MAX_STEPS, moveableSquares);	// SOUTH

//  console.log('checking southwest');
  if(isSouthValid() && isWestValid())
  	goDirection(board, southWest, localPathsFound, stepsTaken, MAX_STEPS, moveableSquares);	// SOUTH WEST

//  console.log('cehcking west');
  if(isWestValid())
  	goDirection(board, west, localPathsFound, stepsTaken, MAX_STEPS, moveableSquares);	// WEST

//  console.log('checking northwest');
  if(isNorthValid() && isWestValid())
  	goDirection(board, northWest, localPathsFound, stepsTaken, MAX_STEPS, moveableSquares);	// NORTH WEST

  return moveableSquares;
}

function goDirection(board, dest, localPathsFound, stepsTaken, MAX_STEPS, moveableSquares){
  stepsTaken++;

  if(dest > -1 && dest < 100){
	  if(!board[dest] || board[dest].piece === null){
	  	let square = indexToNotation(dest, board);
	  	calculateMoveableSquares(square,board, localPathsFound, stepsTaken, MAX_STEPS, moveableSquares);
	  }else{
	  	console.log(dest + ' is  not a good go direction');
	  	console.log(board[dest]);
	  	return false;
	  }
  }
}
